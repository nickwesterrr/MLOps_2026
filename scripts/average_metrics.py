...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="List of history.json files",
    )
...
def main():
    ...
    print("-" * 75)
    print(
        f"{'AVERAGE':<40} | {np.mean(accuracies):.4f}   | {np.mean(f2_scores):.4f}   | {np.mean(roc_aucs):.4f}"
    )
    print(
        f"{'STD DEV':<40} | {np.std(accuracies):.4f}   | {np.std(f2_scores):.4f}   | {np.std(roc_aucs):.4f}"
    )
