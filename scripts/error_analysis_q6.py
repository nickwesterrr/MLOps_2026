...
def parse_args():
    ...
    parser.add_argument(
        "--checkpoint", type=Path, required=True, help="Path to best model checkpoint"
    )
    parser.add_argument(
        "--config", type=Path, required=True, help="Path to config file"
    )
    parser.add_argument(
        "--output_dir", type=Path, default="assets/plots", help="Where to save images"
    )
...
def main():
    ...
    def plot_errors(indices, title, filename):
        ...
        for i, idx in enumerate(indices_to_plot):
            ...
            axes[i].imshow(img)
            axes[i].set_title(
                f"True: {labels[idx]} | Pred: {preds[idx]}\nIntensity: {intensities[idx]:.2f}"
            )
            axes[i].axis("off")
    ...
    plot_errors(
        false_positives,
        "False Positives (Predicted Tumor, Actually Healthy)",
        "q6_false_positives.png",
    )
    plot_errors(
        false_negatives,
        "False Negatives (Predicted Healthy, Actually Tumor)",
        "q6_false_negatives.png",
    )
    ...
    if np.sum(dark_mask) > 0:
        ...
        print(
            f"Dark Slice (N={np.sum(dark_mask)}):  Acc: {dark_acc:.4f} | F2: {dark_f2:.4f}"
        )
    else:
    ...
    if np.sum(bright_mask) > 0:
        ...
        print(
            f"Bright Slice (N={np.sum(bright_mask)}): Acc: {bright_acc:.4f} | F2: {bright_f2:.4f}"
        )
    else:
