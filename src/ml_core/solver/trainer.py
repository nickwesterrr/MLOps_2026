...
class Trainer:
    ...
    def save_checkpoint(self, epoch: int, val_loss: float) -> None:
        if self.tracker_obj:
            filename = Path(
                self.tracker_obj.get_checkpoint_path(f"checkpoint_epoch_{epoch + 1}.pt")
            )
        else:
        ...
        for epoch in range(epochs):
            ...
            if self.tracker_obj:
                metrics_to_log = {
                    ...
                    "grad_norm": self.tracker["grad_norms"][-1]
                    if self.tracker["grad_norms"]
                    else 0.0,
                    "lr": current_lr,
                }
            ...
            print(
                f"Epoch [{epoch + 1}/{epochs}] - LR: {current_lr:.6f} - Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}"
            )
